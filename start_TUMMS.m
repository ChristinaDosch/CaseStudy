...
t = linspace(0,24,N);
x0 = 1;
if nargin == 0, x = x0 + P*pdf(makedist('Normal',12,sqrt(12)),t); end

cost = 1;
penalty = 2; % linear penalty
n = 5000; % number of the day simulations
...
hPanel1 = uipanel('Title', 'Eingabe', 'Visible', 'off', 'Position', [0 0.9 1 0.1]);           % Eingabepanel
pos = linspace(100,s(3)-50,25);                                             
for k = 1:N;
    hEdit(k) = uicontrol('Style', 'edit', 'Parent', hPanel1, 'Visible', 'off', 'String', num2str(x(k)),...
                         'Position', [pos(k) 20 20 20],...
                         'Callback', @update_main_plot);
    hLabel(k) = uicontrol('Style', 'text', 'Parent', hPanel1, 'Visible', 'off', 'String', [num2str(k-1) ':00'],...
...
hSlider = uicontrol('Style', 'slider', 'Visible', 'off', 'BackgroundColor', 'white',...
                    ...
                    'Position', [0 0 100 20],...
                    'Callback', @update_main_plot);               
hLabel_zeit = uicontrol('Style', 'text', 'Parent', hPanel1, 'Visible', 'off', 'String', 'Zeit',...
                        'Position', [0 20 20 20]);
hLabel_wert = uicontrol('Style', 'text', 'Parent', hPanel1, 'Visible', 'off', 'String', 'Wert',...
                        'Position', [0 40 20 20]);
hButton = uicontrol('Style', 'pushbutton', 'Parent', hPanel1, 'Visible', 'off', 'String', 'START',...
                    'Position', [0 0 0 0],...
...
function create_bestworst_plot
    ...
    legend('Tag mit dem kleinsten Gewinn',...
           'Tag mit dem groessten Gewinn',...
           'deine Vorhersage')
    xlabel('Zeit'), ylabel('Energie, MWh')
...
function [obj, i_bad, i_good] = calculate_objective
    ...
    i_bad = find(obj == min(obj));
    i_good = find(obj == max(obj));
end
