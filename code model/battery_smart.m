...
% x \in R^3T is going to be the optimization variable in this function. 
% x = [SOC,b^in,b^out]

x_tilde = @(x) e + 0.95*x(2*T+1:3*T) - x(T+1:2*T);
F = @(x) obj_SO_discr(x_plan,x_tilde(x),cost,penalty,penalty_grad,epsilon,P,penalty_hess); % for a given e and x_plan, the objective is a function of b^in and b^out

x0 = zeros(1,3*T);
%hessianfcn = @(x) hess_SO_discr_smart(x_plan,x_tilde(x),penalty_hess,epsilon,P);
options = optimoptions('fmincon','GradObj','on');%,'Hessian','user-supplied','HessFcn',hessianfcn);
%tic
