...
%for i=1:
[pos_in_1,~, ~] = smooth_ppart(e(1)-x(1), epsilon_smooth, 6);
[m_in_1,~, ~] = smooth_min(pos_in_1,(0.95-SOC_0)*C, epsilon_smooth);
[pos_out_1,~, ~] = smooth_ppart(1/0.95*(x(1)-e(1)), epsilon_smooth, 6);
[m_out_1, ~, ~] = smooth_min(pos_out_1,(SOC_0-0.1)*C, epsilon_smooth);
...
for i = 2:T     
    ...
    [pos_in,~, ~] = smooth_ppart(e(i)-x(i), epsilon_smooth, 6);
    [m_in,~, ~] = smooth_min(pos_in,(0.95-SOC(i-1))*C, epsilon_smooth);
    [pos_out,~, ~] = smooth_ppart(1/0.95*(x(i)-e(i)), epsilon_smooth, 6);
    [m_out, ~, ~] = smooth_min(pos_out,(SOC(i-1)-0.1)*C, epsilon_smooth);
