...
penalty_hess = @(x) 2*ones(x); % Hessian of the penalty function
% penalty = @(x) 5*x; % linear penalty
% penalty_grad = @(x) 5*ones(x); % derivative of the penalty function
% penalty_hess = @(x) zeros(x); % Hessian of the penalty function
